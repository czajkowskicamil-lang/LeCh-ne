  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.fillColor('#FFFFFF').font('Times-Bold').fontSize(22)
        .text("Le compte à rebours de l'après-carrière", M, 48, { width: CW - 150 });
      doc.fillColor(GOLD_LIGHT).font('Times-Italic').fontSize(12)
        .text(d.sportLabel ? d.sportLabel : 'Sportif de haut niveau', M, 104);
      ...
      doc.fillColor(INK).font('Times-Bold').fontSize(33).text(eur(d.nombreLiberte), M + 20, y + 31);
      doc.fillColor(STONE).font('Helvetica').fontSize(9)
        .text('la somme qui, bien placée, vous fait vivre sans salaire, une fois la carrière terminée.', M + 20, y + 68, { width: CW - 40 });
      y += 84 + 18;
      ...
      y += 16;
      const baseY = y + 78, colW = 150;
      const rev = +d.revenu || 0, rente = +d.renteActuelle || 0;
      const hAv = 72, hAp = rev > 0 ? Math.max(8, Math.min(hAv, (rente / rev) * hAv)) : 8;
      ...
      doc.fillColor(STONE).font('Helvetica').fontSize(8.5).text("Votre salaire aujourd'hui", M + 10, baseY + 8, { width: colW, align: 'center' });
      const x2 = M + 10 + colW + 90;
      doc.roundedRect(x2, baseY - hAp, colW, hAp, 4).fill(BLUE);
      doc.fillColor(INK).font('Times-Bold').fontSize(12).text(eur(rente) + ' / an', x2, baseY - hAp - 16, { width: colW, align: 'center' });
      doc.fillColor(STONE).font('Helvetica').fontSize(8.5).text('Si le sport s’arrête demain', x2, baseY + 8, { width: colW, align: 'center' });
      // (rente réelle de l'épargne actuelle, au taux de retrait prudent de 3,5 %)
      ...
      // -------- Chiffres clés --------
      const rows = [
        ['Ce qu\'il vous faut pour vivre', d.depenses != null ? eur(d.depenses) + ' / an' : '—'],
        [
          'Trajectoire de votre épargne actuelle',
          d.capitalFinCarriere != null
            ? eur(d.capitalFinCarriere) + (d.progression != null ? '  (' + d.progression + ' % de la cible)' : '')
            : '—',
        ],
        [
          'Votre date de liberté estimée' + (d.fin != null && d.age != null ? '  (fin de carrière : ' + d.fin + ' ans)' : ''),
          d.dateLiberte || '—',
        ],
        ["À mettre de côté / mois d'ici la fin de carrière", d.versementNecessaire != null ? eur(d.versementNecessaire) + ' / mois' : '—'],
        ['Ce que vous pouvez mettre de côté / mois', d.capaciteMensuelle != null ? eur(d.capaciteMensuelle) + ' / mois' : '—'],
      ];
      rows.forEach((r) => {
      ...
      doc.fillColor(INK).font('Times-Bold').fontSize(13).text('Transformons ce compte à rebours en stratégie.', M + 18, y + 14, { width: CW - 36 });
      doc.fillColor(STONE).font('Helvetica').fontSize(9.5)
        .text("Un premier échange de 30 minutes, sans engagement, pour bâtir votre plan d'après-carrière.", M + 18, y + 34, { width: CW - 36 });
